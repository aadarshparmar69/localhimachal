          ...
          <div className="absolute inset-0 flex items-end">
            <div className="container mx-auto px-4 lg:px-8 pb-16">
              ...
              >
                ...
                <div className="inline-block px-4 py-1.5 rounded-full bg-accent/20 backdrop-blur-sm text-accent text-sm font-body mb-4">
                  Hidden Place
                </div>
        ...
        <section className="py-16 bg-background">
          <div className="container mx-auto px-4 lg:px-8">
            <div className="max-w-4xl mx-auto">
              ...
              <ScrollReveal>
                <div className="prose prose-lg max-w-none mb-16">
                  {place.emotionalOpening.split('\n\n').map((para, i) => (
                    <p key={i} className="font-body text-lg text-muted-foreground leading-relaxed mb-6 first-letter:text-5xl first-letter:font-display first-letter:text-primary first-letter:float-left first-letter:mr-3 first-letter:mt-1">
                      {para}
                    </p>
                  ))}
                </div>
              ...
              {/* Why Hidden */}
              <ScrollReveal delay={0.1}>
                <div className="mb-16 p-8 rounded-2xl bg-secondary/50">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-16">
                  <div className="p-6 rounded-xl bg-card shadow-soft">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="font-display text-lg font-semibold text-foreground">Best Time to Visit</h3>
                    </div>
                    <p className="font-body text-sm text-muted-foreground">{place.bestTimeToVisit}</p>
                  </div>
                  <div className="p-6 rounded-xl bg-card shadow-soft">
                    <div className="flex items-center gap-2 mb-4">
                    ...
                    <p className="font-body text-sm text-muted-foreground">{place.howToReach}</p>
                  </div>
                </div>
              </ScrollReveal>
              ...
              {/* Responsible Travel */}
              <ScrollReveal delay={0.1}>
                <div className="mb-16 p-8 rounded-2xl bg-destructive/5 border border-destructive/20">
                        ...
                        <div className="space-y-3">
                          {nearbyTreks.map((trek) => trek && (
                            <Link
                              key={trek.id}
                              to={`/trek/${trek.slug}`}
                              className="block p-4 rounded-xl bg-card hover:shadow-card transition-shadow"
                            >
                              <div className="font-display font-semibold text-foreground">{trek.name}</div>
                              <div className="font-body text-sm text-muted-foreground">{trek.difficulty} • {trek.duration}</div>
                            </Link>
                          ))}
                        </div>
                      </div>
