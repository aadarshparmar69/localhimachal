  ...
  return (
    <section ref={sectionRef} className="py-20 sm:py-28 lg:py-32 bg-muted/30 overflow-hidden relative">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <StaggerContainer 
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mb-14 sm:mb-16"
          staggerDelay={0.1}
          delayChildren={0.2}
        >
          {featuredDistricts.map((district, index) => (
            <StaggerItem key={district.id} direction={index % 2 === 0 ? "left" : "right"}>
              <motion.div
              ...
              >
                <Link
                  ...
                  className="group block relative aspect-[4/3] rounded-2xl overflow-hidden shadow-card hover:shadow-elevated transition-shadow duration-500"
                >
                  <motion.img
                    src={district.image}
                    alt={district.name}
        ...
        >
          <Link 
            to="/explore" 
            className="group inline-flex items-center gap-3 px-7 py-3.5 rounded-full bg-primary/5 hover:bg-primary/15 border border-primary/20 text-foreground font-medium transition-all duration-300"
          >
