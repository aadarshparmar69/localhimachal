...
export const IntroSection = () => {
  ...
  const lines = ["This is not a travel guide.", "It's a doorway into the soul of a place where ancient temples whisper stories,", "where village elders share wisdom over chai,", "and where every mountain path leads to something unexpected."];
  return <section ref={containerRef} className="relative py-20 sm:py-28 lg:py-32 bg-background overflow-hidden">
      {/* Animated background elements */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div style={{
        opacity: prefersReducedMotion ? 0.04 : opacity
      ...
      scale: prefersReducedMotion ? 1 : smoothScale
        ...
        <div className="text-center mb-12 sm:mb-16">
          ...
          <StaggerContainer className="space-y-3 sm:space-y-4" staggerDelay={0.12}>
            {lines.map((line, index) => <StaggerItem key={index}>
        ...
        ease: [0.22, 1, 0.36, 1]
          ...
          <Link to="/culture" className="group flex items-center gap-2 text-foreground font-medium hover:text-primary transition-all duration-300">
          ...
          <Link to="/plan" className="group flex items-center gap-2 text-foreground font-medium hover:text-primary transition-all duration-300">
            <motion.span whileHover={{
            x: -6
          }} transition={{
            type: "spring",
